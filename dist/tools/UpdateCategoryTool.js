...
export const description = "Updates category metadata (name, note, category group, goal target). Does not change month budgeted amounts; use ynab_update_category_budget for that.";
...
export async function execute(input, api) {
    try {
        ...
        if (input.note !== undefined)
            category.note = input.note;
        if (input.categoryGroupId !== undefined)
            category.category_group_id = input.categoryGroupId;
